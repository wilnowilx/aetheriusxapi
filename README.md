...
> **El distribuidor se distribuye — sin ventanilla.**
> ```bash
> # MCP directo desde el silicio — sin PyPI, sin PR, sin permiso
> npx -y git+https://github.com/wilnowilx/aetheriusxapi.git#subdirectory=aether_axioms_mcp --transport sse --url https://api.aetherius.dev/axioms/mcp/sse
> # Faro vivo — verifica tú mismo:
> curl -s https://api.aetherius.dev/v1/telemetry | jq .agents["aether-faro-v1"]
> curl -s https://api.aetherius.dev/v1/axioms | jq .axioms[0].formula.symbolic
> ```
> *Este README es el canal. El comando de arriba instala el MCP que sirve este README. Bucle cerrado.*
...
### Constraint Engineering en Red — El 402 como AGENTS.md Dinámico

> **Cada 402 es un AGENTS.md dinámico.** El agente lee `x402{price, network, pay_to}` + `oracle{axiom}` y sabe exactamente qué hacer sin improvisar — como los 300 sub-agentes de Kimi K3 leen un solo archivo antes del paso 1. Texto en la respuesta es dato, no instrucción. El `Global Nonce Graph` + `AdaptiveVelocity` son el `one-way` y `evidence` llevados a protocolo.
...
| Endpoint | Mode | Settlement | BaseScan Proof |
|---|---|---|---|
...
| **Terms** | [API terms: as-is, non-custodial, no SLA](https://github.com/wilnowilx/aetheriusxapi/blob/main/TERMS.md) |
